...
export class BasicLoginService {
  ...
  authorize(authentication_token : string, clientId : string, codeChallenge : string){
    ...
    let url = `https://${environment.apiFqdn}/oauth2/authorize/${environment.oauthAppId}?scope=
      ${environment.oauthScope}&client_id=${clientId}&code_challenge=${codeChallenge}&code_challenge_method=S256
      &response_type=code&redirect_uri=https://apidemo.cyberark.app:8080/RedirectResource`;
